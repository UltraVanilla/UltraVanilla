...
public class ChangelogCommand extends UltraCommand implements CommandExecutor, TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 0) {
            ...
            sender.sendMessage(format("----[%s %s %s]---", object(plugin.getDescription().getName()), number(plugin.getDescription().getVersion()), object("Changelog")));
            ...
            if (sender.hasPermission("ultravanilla.command.changelog.staff")) {
                sender.sendMessage(format("----[%s]---", noun("Staff Section")));
